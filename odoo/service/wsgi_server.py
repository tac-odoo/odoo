...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, odoo.osv.orm.except_orm): # legacy
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, odoo.tools.ustr(e.value))
        response = xmlrpclib.dumps(fault, allow_none=False, encoding=None)
    elif isinstance(e, odoo.exceptions.Warning) or isinstance(e, odoo.exceptions.RedirectWarning):
...
def xmlrpc_handle_exception_string(e):
    if isinstance(e, odoo.osv.orm.except_orm):
