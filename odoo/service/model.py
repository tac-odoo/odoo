        ...
        while True:
            try:
                ...
                for key in registry._sql_error.keys():
                    if key in inst[0]:
                        raise odoo.osv.orm.except_orm(_('Constraint Error'), tr(registry._sql_error[key], 'sql_constraint') or inst[0])
                if inst.pgcode in (errorcodes.NOT_NULL_VIOLATION, errorcodes.FOREIGN_KEY_VIOLATION, errorcodes.RESTRICT_VIOLATION):
                    ...
                    try:
                        ...
                        pass
                    raise odoo.osv.orm.except_orm(_('Integrity Error'), msg)
                else:
                    raise odoo.osv.orm.except_orm(_('Integrity Error'), inst[0])

    return wrapper

def execute_cr(cr, uid, obj, method, *args, **kw):
    object = odoo.registry(cr.dbname).get(obj)
    if object is None:
