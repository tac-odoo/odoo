    ...
    def postprocess_and_fields(self, cr, user, model, node, view_id, context=None):
        ...
        if node.tag == 'diagram':
            if node.getchildren()[0].tag == 'node':
                ...
                if not node.get("create") and not node_model.check_access_rights(cr, user, 'create', raise_exception=False) and not context.get("create",True):
        ...
        if node.tag in ('kanban', 'tree', 'form', 'gantt'):
            for action, operation in (('create', 'create'), ('delete', 'unlink'), ('edit', 'write')):
                if not node.get(action) and not Model.check_access_rights(cr, user, operation, raise_exception=False) or not context.get(action, True):
            ...
            if group_by_name in Model._fields:
                ...
                if group_by_field.type == 'many2one':
                    ...
                    for action, operation in (('group_create', 'create'), ('group_delete', 'unlink'), ('group_edit', 'write')):
                        if not node.get(action) and not group_by_model.check_access_rights(cr, user, operation, raise_exception=False) and not context.get(action,True):
                            node.set(action, 'false')
