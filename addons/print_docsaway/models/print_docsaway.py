        ...
        Explaination of the Response JSON Object:
            ...
            - transaction: propoerties of the transaction, 5 subfields:
                ...
                - Balance: return the remaining account balance after
                    transaction (in AUD)
        """
        APIMode = 'LIVE' if self.environment == 'production' else 'TEST'
        # build request
        request = {
            'APIMode': APIMode,
            'Recipient': {
    ...
    def _docsaway_action_compute_price(self):
        ...
        for order in self:
            ...
            if not (key in prices):
                prices[key] = order.provider_id._docsaway_fetch_price(order.docsaway_station, order.docsaway_courier, order.docsaway_zone, order.currency_id.name, order.nbr_pages, order.ink, order.paper)
            response = prices[key]
            order.write({
