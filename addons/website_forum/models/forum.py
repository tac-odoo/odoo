...
class Forum(models.Model):
    ...
    introduction_message = fields.Html(
    ...
    def _check_default_post_type(self):
        if self.default_post_type == 'link' and not self.allow_link or self.default_post_type == 'question' and not self.allow_question or self.default_post_type == 'discussion' and not self.allow_discussion:
            raise Warning('Post type in "Default post" must be activated')

    def get_tags_char(self):
        col = 'name'
        if self.env.registry.has_unaccent:
            col = 'unaccent(name)'
        self._cr.execute("""
            SELECT UPPER(SUBSTR(%s,1,1)) as first_char,count(name)
            FROM   forum_tag
            WHERE forum_id = %s
            GROUP  BY UPPER(SUBSTR(%s,1,1))
            ORDER BY first_char;
        """%(col,self.id,col))
