...
class TestSalemembership(TestMembershipSale):
    ...
    def test_membership_sale(self):
        ...
        membership_line_ids = self.membership_line.search([('sale_order_id', '=', order_id1.id)])
        self.assertTrue(membership_line_ids, 'Membership Line: Creation of Membership Line failed.')
        self.assertEqual(
        ...
        self.assertTrue(membership_line_ids.account_invoice_line, 'Invoice is not confirmed.')
