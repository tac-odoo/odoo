...
class lunch_order(models.Model):
    ...
    def add_preference(self, pref_id):
        ...
        """
        self.ensure_one()
        orderline_obj = self.env['lunch.order.line']
        pref = orderline_obj.browse(pref_id)
        new_order_line = {
            'date': self.date,
        ...
        return orderline_obj.create(new_order_line)
    ...
    def _default_alerts_get(self):
        ...
        alert_msg = [alert.get_alert_message()
                     for alert in alert_ref.search([]) if alert.get_alert_message()]
        return '\n'.join(alert_msg)
    ...
    alerts = fields.Text(
        compute='_alerts_get', string="Alerts", default=_default_alerts_get)
