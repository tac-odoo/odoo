...
$.widget( "evol.colorpicker", {
			...
			case 'INPUT':
				...
				e.addClass('colorPicker '+this._id)
					.wrap('<div style="'
						...
						+'"></div>')
					.parent()
					.before('<div class="evo-colorind'+(isMoz?'-ff':_ie)+'" '+
				...
				if(showOn=='both' || showOn=='button'){
					e.parent().prev().on('click', function(evt){
						evt.stopPropagation();
						that.showPalette();
					});
				}
	...
	showPalette: function() {
		if(this._enabled){
			...
			if(this._palette==null){
				this._palette=this.element.parent().prev()
	...
	_setValue: function(c, noHide) {
		...
		if(this._isPopup){
			...
			this.element.val(c)
				.parent().prevAll('.evo-colorind-ff').attr('style', 'background-color:'+c);
		}else{
	...
	destroy: function() {
		...
		if(this._isPopup){
			this.element
				.parent().prev().off('click').remove()
				.end().children('input').off('focus').unwrap();						
		}
		this.element.removeClass('colorPicker '+this._id).empty();
