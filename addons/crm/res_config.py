...
class crm_configuration(models.TransientModel):
    ...
    def _find_default_lead_alias_id(self):
        alias_id = self.env['ir.model.data'].xmlid_to_res_id('crm.mail_alias_lead_info')
        if not alias_id:
    ...
    def set_default_alias_prefix(self):
        ...
        for record in self:
            ...
            if not alias_id:
                ...
                alias_id = mail_alias.create(self._cr, self._uid, {'alias_name': record.alias_prefix}, context=self._context)
            else:
                mail_alias.write(self._cr, self._uid, alias_id, {'alias_name': record.alias_prefix}, context=self._context)
        return True