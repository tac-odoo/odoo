...
class test_mail_access_rights(TestMail):
    ...
    def test_00_message_read(self):
        ...
        no_of_notify = self.MailNotificationObj.search_count([('is_read', '=', False), ('message_id', 'in', msg_ids), ('partner_id', '=', user_raoul.partner_id.id)])
        self.assertEqual(no_of_notify, 11, 'message_post: wrong number of produced notifications')

        # Test: read some specific ids 
        messages  = self.MailMessageObj.sudo(user_raoul.id).with_context({'mail_read_set_read': True}).message_read(msg_ids[2:4], domain=[('body', 'like', 'dummy')])
        ...
        messages = self.MailMessageObj.sudo(user_raoul.id).message_read(domain=pigs_domain, limit=200)
        read_msg_ids = [msg.get('id') for msg in messages]
        self.assertEqual(msg_ids, read_msg_ids, 'message_read flat with domain on Pigs should equal all messages of Pigs')
        messages = self.MailMessageObj.sudo(user_raoul.id).message_read(domain=pigs_domain, limit=200, thread_level=1)
        read_msg_ids = [msg.get('id') for msg in messages]
        ...
        messages = self.MailMessageObj.message_read(domain=pigs_domain, limit=1, thread_level=1)
        ...
        messages = self.MailMessageObj.message_read(domain=domain, limit=2, thread_level=0, parent_id=msg_id2)
        read_msg_ids = [msg.get('id') for msg in messages if msg.get('type') != 'expandable']
        ...
        messages = self.MailMessageObj.message_read(domain=new_msg_exp.get('domain'), limit=2, thread_level=0, parent_id=msg_id2)
        read_msg_ids = [msg.get('id') for msg in messages if msg.get('type') != 'expandable']
        ...
        messages = self.MailMessageObj.message_read(domain=domain, limit=1, thread_level=1)
        read_msg_ids = [msg.get('id') for msg in messages if msg.get('type') != 'expandable']
        # Test: structure content, ancestor is added to the read messages, ordered by id, ancestor is set, 2 expandables
        ...
        messages = self.MailMessageObj.message_read(domain=domain, limit=200, thread_level=0, parent_id=msg_id1)
        read_msg_ids = [msg.get('id') for msg in messages if msg.get('type') != 'expandable']
        ...
        messages = self.MailMessageObj.message_read(domain=pigs_domain, limit=2, thread_level=0)
        read_msg_ids = [msg.get('id') for msg in messages if msg.get('type') != 'expandable']
        ...
        messages = self.MailMessageObj.message_read(domain=domain, limit=20, thread_level=0)
        read_msg_ids = [msg.get('id') for msg in messages if msg.get('type') != 'expandable']
        # Test: structure content, ancestor is added to the read messages, ordered by id, ancestor is set, 2 expandables
