...
class mail_thread(osv.AbstractModel):
    ...
    @api.multi
    def get_subscription_data(self, partner_id=None, context=None):
        """ Computes message_subtype_data: data about document subtypes: which are
    ...
    def message_redirect_action(self):
        ...
        if msg_id and not (model and res_id):
            msg = self.env['mail.message'].browse(cr, uid, msg_id, context=context)
            if msg.exists():
...
# >>>>>>> [WIP] migrate
    ...
    def message_route_verify(self, message, message_dict, route, update_author=True, assert_model=True, create_fallback=True, allow_private=False):
        ...
        def _warn(message):
            _logger.warning('Routing mail with Message-Id %s: route %s: %s', message_id, route, message)
