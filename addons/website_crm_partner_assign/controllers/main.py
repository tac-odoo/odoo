...
class WebsiteCrmPartnerAssign(WebsitePartnerPage):
    ...
    def partners_detail(self, partner_id, partner_name='', **post):
        response = super(WebsiteCrmPartnerAssign, self).partners_detail(partner_id, partner_name, **post)
        # Now we can redirect to the /partners page instead
        # TODO: Find a prettier way to check the response
        if str(response) == str(request.not_found()):
