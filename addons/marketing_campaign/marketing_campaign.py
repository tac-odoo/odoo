...
class marketing_campaign_activity(osv.osv):
  ...
  """),
    ...
    def _process_wi_report(self, cr, uid, activity, workitem, context=None):
        report_data, format = render_report(cr, uid, [workitem.res_id], activity.report_id.report_name, None, context=context)
        attach_vals = {
            'name': '%s_%s_follower_%s'%(activity.report_id.report_name,
                                activity.name,workitem.id),
            'datas_fname': '%s.%s'%(activity.report_id.report_name,
                                        format),
            'parent_id': activity.report_directory_id.id,
            'datas': base64.encodestring(report_data),
            'file_type': activity.report_id.report_type
        }
