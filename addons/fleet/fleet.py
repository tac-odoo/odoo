...
class fleet_vehicle(models.Model):
    ...
    def _vehicle_name_get_fnc(self):
        for record in self:
            if record.model_id:
                record.name = record.model_id.make_id.name + ' / ' + record.model_id.modelname + ' / ' \
...
class fleet_vehicle_log_contract(models.Model):
    ...
    def scheduler_manage_auto_costs(self):
        ...
        for contract in contract_ids:
            ...
            while (last_cost_date <= today) and (last_cost_date <= fields.Datetime.from_string(contract.expiration_date)):
                data = {
    ...
    expiration_date = fields.Date('Contract Expiration Date',
                                  ...
                                  help='Date when the coverage of the contract expirates (by default, '
                                       'one year after begin date)')
    ...
    generated_cost_ids = fields.One2many('fleet.vehicle.cost', 'contract_id',
                                         string='Generated Costs')
    sum_cost = fields.Float(compute='_get_sum_cost', string='Indicative Costs Total')
    cost_id = fields.Many2one('fleet.vehicle.cost', string='Cost',
