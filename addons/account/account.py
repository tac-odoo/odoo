...
=======
    def _get_financial_report_line_ref(self, cr, uid, context=None):
        obj_data = self.pool.get('ir.model.data')
        obj_financial_report_line = self.pool.get('account.financial.report.line')
        financial_report_line_ref = {}
        for key, financial_report_line in [
            ...
            try:
                financial_report_line_ref[key] = obj_financial_report_line.browse(cr, uid,
                    obj_data.get_object_reference(cr, uid, 'account', financial_report_line)[1],
                    context=context)
            except ValueError:
                pass
        return financial_report_line_ref

    def _get_current_report_type(self, cr, uid, ids, name, arg, context=None):
        res = {}
        financial_report_line_ref = self._get_financial_report_line_ref(cr, uid, context=context)
        for record in self.browse(cr, uid, ids, context=context):
            res[record.id] = 'none'
            for key, financial_report_line in financial_report_line_ref.items():
                list_ids = [x.id for x in financial_report_line.account_type_ids]
    ...
    def _save_report_type(self, cr, uid, account_type_id, field_name, field_value, arg, context=None):
        ...
        for key, financial_report_line in financial_report_line_ref.items():
            ...
            if account_type_id in list_ids:
                obj_financial_report_line.write(cr, uid, [financial_report_line.id], {'account_type_ids': [(3, account_type_id)]})
        #write it in the good place
        if field_value != 'none':
            return obj_financial_report_line.write(cr, uid, [financial_report_line_ref[field_value].id], {'account_type_ids': [(4, account_type_id)]})
