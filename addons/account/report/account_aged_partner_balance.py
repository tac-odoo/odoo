...
class aged_trial_report(report_sxw.rml_parse, common_report_header):
    ...
    def _get_lines(self, form):
        ...
        self.cr.execute('SELECT DISTINCT res_partner.id AS id,\
                ...
                WHERE (l.account_id=account_account.id) \
                    ...
                    AND (am.state IN %s)\
                    AND (at.type IN %s)\
                    AND account_account.deprecated is False\
        ...
        self.cr.execute('SELECT l.partner_id, SUM(l.debit-l.credit) \
                    ...
                    OR (l.reconcile_id IN (SELECT recon.id FROM account_move_reconcile AS recon WHERE recon.create_date > %s )))\
                     ' + self.query + '\
                    AND account_account.deprecated\
                    AND (l.date <= %s)\
    ...
    def _get_lines_with_out_partner(self, form):
        ...
        self.cr.execute('SELECT SUM(l.debit-l.credit) \
                    ...
                    ' + self.query + '\
                    AND (l.date <= %s)\
                    AND account_account.deprecated ',(tuple(move_state), tuple(self.ACCOUNT_TYPE), self.date_from, self.date_from,))
        t = self.cr.fetchall()
        for i in t:
