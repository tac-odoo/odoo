...
class account_invoice_report(models.Model):
    ...
    def _compute_amounts_in_user_currency(self):
        ...
        currency_rate = currency_rate_obj.search([('rate', '=', 1)], limit=1)[0]
        base_currency = currency_rate.currency_id
        res = {}
        context = self._context.copy()
        context['date'] = item.date
        self.user_currency_price_total = base_currency.with_context(context).compute(user_currency_id)
        self.user_currency_price_average = base_currency.with_context(context).compute(user_currency_id)
        self.user_currency_residual = base_currency.with_context(context).compute(user_currency_id)
