...
class TestHrMaterial(TransactionCase):
    ...
    def test_hr_material_request_category(self):
        ...
        maintenance_request_value = {
            ...
            'category_id': self.ref('hr_material.hr_material_monitor'),
            'color': 7,
            'stage_id': self.ref('hr_material.stage_new')
        }
        ...
        self.assertEquals(maintenance_request_01.stage_id.id, self.ref('hr_material.stage_new'))

        # I check that change the maintenance_request stage on click statusbar
        maintenance_request_01.sudo(self.hr_user).write({'stage_id': self.ref('hr_material.stage_in_progress')})

        # I check that maintenance request is in the "In Progress" stage
        self.assertEquals(maintenance_request_01.stage_id.id, self.ref('hr_material.stage_in_progress'))
