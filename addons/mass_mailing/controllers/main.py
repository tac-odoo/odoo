...
class MassMailController(http.Controller):
    ...
    def unsubscribe_page(self, mailing_id, email=None, res_id=None, **post):
        cr, context = request.cr, request.context
        contacts = request.registry['mail.mass_mailing.contact'].search_read(cr, SUPERUSER_ID, [('email', 'ilike', email), ('opt_out', '=', False)], ['list_id'], context=context)
        if len(contacts) == 0:
            contacts = False
        print contacts
        return request.website.render('mass_mailing.page_unsubscribe', {'contacts': contacts, 'mailing_id': mailing_id, 'email': email})

    @http.route(['/mail/mailing/unsubscribe'], type='http', auth='none', website=True)
    def unsubscribe(self, **post):
        cr, context = request.cr, request.context
        contact_ids = [int(post[key]) for key in post.keys()]
        request.registry['mail.mass_mailing.contact'].write(cr, SUPERUSER_ID, contact_ids, {'opt_out': True}, context=context)
