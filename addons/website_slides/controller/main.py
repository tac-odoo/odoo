...
class main(http.Controller):
    ...
    def slide_thumb(self, document_id=0, **post):
        ...
        Files = request.env['ir.attachment']
        Website = request.env['website']
        user = Files.search([('id', '=', document_id)])
        return Website._image('ir.attachment', user.id, 'image', response)
    ...
    def slide_like(self, channel, slideview, **post):
        ...
        if slideview.write({'likes':likes}):
            return likes
