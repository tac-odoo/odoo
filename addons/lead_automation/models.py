...
class lead_automation_campaign(models.Model):
	...
	state= fields.Selection([('draft', 'New'),
 	...
 	activity_ids= fields.One2many('lead.automation.activity', 'campaign_id', 'Activities')
 	...
 	segment_ids= fields.One2many('lead.automation.segment', 'campaign_id', 'Segments', readonly=False)
	...
	def action_run(self):
		if not self.activity_ids:
			raise exceptions.ValidationError("A Campaign without any Activity cannot be set to Run.")
		...
		if not has_start and not has_signal_without_from:
			raise exceptions.ValidationError("The campaign cannot be started. It does not have any starting activity. Modify campaign's activities to mark one as the starting point.")
	...
	def action_done(self):
		...
		if has_active_segment:
			raise exceptions.ValidationError('The campaign cannot be marked as done before all segments are closed.')
		self.state = 'done'
...
class lead_automation_segment(models.Model):
	...
	name= fields.Char('Name', required=True)
	campaign_id= fields.Many2one('lead.automation.campaign', 'Campaign', required=True, select=1, ondelete="cascade")
	object_id= fields.Many2one(related='campaign_id.object_id', string='Resource',store=True)
...
class lead_automation_activity(models.Model):
	...
	object_id= fields.Many2one(related='campaign_id.object_id', string='Object', readonly=True)
...
class lead_automation_transition(models.Model):
	...
	activity_from_id= fields.Many2one('lead.automation.activity','Previous Activity', select=1,required=True, ondelete="cascade")
	activity_to_id= fields.Many2one('lead.automation.activity','Next Activity',required=True, ondelete="cascade")
