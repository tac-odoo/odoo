...
class purchase_requisition_partner(osv.osv_memory):
    ...
    _columns = {
        'partner_id': fields.many2many('res.partner', 'purchase_requisition_supplier_rel', 'requisition_id', 'partner_id', string='Suppliers', required=True, domain=[('supplier', '=', True)])
    }
    ...
    def create_order(self, cr, uid, ids, context=None):
        ...
        for partner_id in self.browse(cr, uid, ids, context=context).partner_id:
            self.pool.get('purchase.requisition').make_purchase_order(cr, uid, active_ids, partner_id.id, context=context)
